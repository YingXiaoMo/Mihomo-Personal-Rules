...
"""
train_smart.py: LightGBM Model Trainer for Mihomo Smart Node Selection (Final Fix)

主要修复：
1. 自动识别数据量，如果真实测速数据不足，自动回退到模拟官方权重，防止模型崩溃。
2. 移除重复的 Log1p 变换，防止数值被过度压缩。
3. 补全 RobustScaler 参数保存，防止路由器读取模型出错。
4. 增强数据清洗逻辑，只针对数值列处理。
"""
