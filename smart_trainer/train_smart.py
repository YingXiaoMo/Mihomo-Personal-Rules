...
def preprocess_data(df: pd.DataFrame, feature_order: dict) -> tuple:
    ...
    logging.info(f"Filtered {filtered_rows} rows with invalid target values (NaN). Remaining rows: {final_rows}")
    
    if df.empty:
        raise ValueError("No data left after dropping NaNs in target column.")
    y = df[TARGET_MAIN]
