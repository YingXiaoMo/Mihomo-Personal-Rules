...
def main():
    print_separator("Mihomo Smart Trainer")
    ...
    try: feature_order = get_feature_order()
    except Exception as e: print(f"❌ 初始化失败: {e}"); return

    try: df = load_data(args.data_dir)
    ...
    if final_score > 8.0: print(" 🏆 评级: S级 (极佳) - 极速节点识别精准")
    elif final_score > 6.0: print(" 🟢 评级: A级 (良好) - 模型可用性高")
    elif final_score > 4.0: print(" 🟡 评级: B级 (及格) - 正常水平")
    else: print(" 🟠 评级: C级 (一般) - 需积累更多数据")
