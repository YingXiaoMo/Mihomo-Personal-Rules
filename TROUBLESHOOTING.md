...
## 1. 数据漂移与时效性
**问题描述**: 
网络环境是动态变化的。如果使用过旧的数据进行训练，模型可能会学习到已经过时的网络特征（例如某个 IP 段或者节点曾经很快，现在变慢了），导致预测准确率下降。
...
- 还有开启“一边使用一边收集”后，模型倾向于只选老节点，新节点永远没机会上位。  

**根本原因**:  
- 模型过度依赖“下载流量”这个数据。但是只有被选中的节点才有流量，没被选中的流量为 0，导致模型认为没被选中过的节点“能力不行”，陷入“强者恒强”的恶性循环。  

**解决方案**:
- **引入计数特征**: 在特征工程中明确保留成功连接次数和失败次数特征。
...
## 6. R² 为 NaN 或训练失败 (NaN/Inf 错误)
**问题描述**: 
在某些情况下，Mihomo 导出的 CSV 文件中，`download_mbps` 或其他关键指标可能包含非数字 (`NaN`) 或无穷大 (`Inf`) 的值。这会导致 LightGBM 训练过程中出现错误，或者计算出的 R² 分数为 NaN。

**解决方案**:
- **数据清洗增强**: 训练脚本在预处理阶段增加了对 `NaN` 和 `Inf` 值的检查和填充。
- **目标变量校验**: 确保作为训练目标的 `maxdownloadrate_kb` (或 `download_mbps`) 是有效的数值类型，如果整列数据无效，脚本会抛出明确的错误而不是静默失败。
